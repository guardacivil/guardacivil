...
if (isset($_GET['download_pdf']) && is_numeric($_GET['download_pdf'])) {
    ...
    if (in_array($perfil, ['Comando', 'Secretário', 'Administrativo', 'Administrador', 'Comandante Geral'])) {
        $stmt = $pdo->prepare("SELECT * FROM partes WHERE id = ?");
        $stmt->execute([$id]);
        $parte = $stmt->fetch();
        if ($parte) {
...
// Apenas Comando, Secretário, Administrativo ou Administrador podem acessar
if (!in_array($perfil, ['Comando', 'Secretário', 'Administrativo', 'Administrador', 'Comandante Geral'])) {
...
if (
    ...
    isset($_POST['responder_parte']) &&
    in_array($perfil, ['Comando', 'Secretário', 'Administrativo', 'Administrador', 'Comandante Geral'])
) {
    $id = intval($_POST['parte_id']);
    $resposta = trim($_POST['resposta']);
    if ($id && $resposta) {
        $stmt = $pdo->prepare('INSERT INTO parte_respostas (parte_id, usuario_id, usuario_nome, resposta, data_resposta) VALUES (?, ?, ?, ?, NOW())');
        $stmt->execute([$id, $currentUser['id'], $currentUser['nome'], $resposta]);
...
// Buscar partes recebidas
$stmt = $pdo->query("SELECT * FROM partes ORDER BY data_criacao DESC");
...
<head>
    ...
    <title>Partes Recebidas - Sistema Integrado da Guarda Civil</title>
...
<body class="bg-gray-100">
    <main class="max-w-3xl mx-auto bg-white p-8 mt-10 rounded shadow">
        <div class="mb-4 flex items-center">
            ...
            <h2 class="text-2xl font-bold mb-0 flex items-center"><i class="fas fa-envelope-open-text mr-2"></i>Partes Recebidas</h2>
        </div>
        <?php if (empty($partes)): ?>
            <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-6">Nenhuma parte recebida.</div>
        <?php else: ?>
                ...
                <tbody>
                    <?php foreach ($partes as $parte): ?>
                        <tr>
                            ...
                            <td class="border px-4 py-2"><?= htmlspecialchars($parte['do_nome']) ?></td>
